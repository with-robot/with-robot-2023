...
class TurtlesimRotateClient(Node):
    ...
    def send_goal(self, theta):
        ...
        goal_future = self.client.send_goal_async(goal_req, 
    ...
    def result_callback(self, future):
        ...
        self.get_logger().info(f'recv feedback: {res.delta}')
    ...
    def cancel_callback(self, future):
        ...
        if len(result_handle.goals_canceling) > 0:
            self.get_logger().info('cancelling goal succeeded.')
        else:
            self.get_logger().info('cancelling goal failed.')
