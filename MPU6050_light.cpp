...
uint8_t MPU6050::begin(TwoWire &w, int gyro_config_num, int acc_config_num){  
...
uint8_t MPU6050::writeData(byte reg, byte data){
  wire->beginTransmission(address);
  wire->write(reg);
  wire->write(data);
  uint8_t status = wire->endTransmission();
  //Serial.printf("writeData: %u\n", status);  
...
void MPU6050::fetchData(){
  ...
  uint8_t bytesReceived =wire->requestFrom(address,(uint8_t) 14);
