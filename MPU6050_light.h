...
class MPU6050{
  public:
	...
	//MPU6050(TwoWire &w);
  MPU6050();    
    uint8_t begin(TwoWire &w, int gyro_config_num=1, int acc_config_num=0);
    //byte begin(int gyro_config_num=1, int acc_config_num=0);
	
	uint8_t writeData(byte reg, byte data);
    ...
    float getDiffAngleZ(){ return diffAngleZ; };
  ...
  private:
    TwoWire *wire;
	uint8_t address = MPU6500_ADDR; // 0x68 or 0x69
